...
// Subsequences with q_len <= SMALL_Q_THRESHOLD and past_len > 0 are routed to
// pa_single_token (which now handles q_len >= 1). Bound matches the typical
// EAGLE/draft spec_num = 16 and the Q_head_chunk_size × KV_PARTITION_STEP_NUM
// register budget. q_len > SMALL_Q_THRESHOLD subsequences fall through to
// pa_multi_token whose tile design covers long-q prefill efficiently.
...
struct PagedAttentionRuntimeParams : public ImplRuntimeParams {
    ...
    size_t single_token_selected_count = 0;  // Total tokens routed to single-token kernel
...
enum PagedAttentionInternBuffIdx {
    ...
    SINGLE_TOKEN_SELECTED_Q_MAPPING = 3,  // 3: i32 pairs [orig_seq_idx, q_in_subseq] for single-token dispatch
