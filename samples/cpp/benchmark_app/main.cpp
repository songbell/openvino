...
static void next_step(const std::string additional_info = "") {
    thread_local static size_t step_id = 0;
...
int main(int argc, char* argv[]) {
    ...
    auto mainloop = [&](std::string targetdevice) -> int {
        ...
        try {
            ...
            std::string device_name = targetdevice;
            ...
            if ((targetdevice.find("GPU") != std::string::npos) && !FLAGS_c.empty()) {
            ...
            if (FLAGS_load_from_file && !isNetworkCompiled) {
                ...
                next_step();
                startTime = Time::now();
                // accept the thread parameters and trying to load to GPU.0, GPU.1, etc
                std::cout << "loading to " << device_name << std::endl;
            ...
            if (perf_counts) {
                ...
                for (size_t ireq = 0; ireq < nireq; ireq++) {
                    ...
                    if (FLAGS_pc) {
                        ...
                        printPerformanceCounts(reqPerfCounts, std::cout, getFullDeviceName(core, targetdevice), false);
                    }
            ...
            slog::info << "HW Device:  " << targetdevice << slog::endl;
    ...
    std::string deviceList = FLAGS_d;
    std::vector<std::string> targetDevice;
    int threadNum = 1;
    if (deviceList.find("MULTITHREAD") != std::string::npos) {
        targetDevice = parse_devices(deviceList);
        threadNum = targetDevice.size();
    } else {
        targetDevice.push_back(deviceList);
    }
    std::thread threads[threadNum];
    // create 2 threads for iGPU + dGPU
    for (int i = 0; i < threadNum; i++) {
        threads[i] = std::thread(mainloop, targetDevice[i]);
    }
